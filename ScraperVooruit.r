...
link = "https://nieuws.vooruit.org/?page="
baselink = "https://nieuws.vooruit.org"
page = read_html(link)
...
for (j in seq(1,NoOfPage)){
  ...
  articleLinks = pageLinkNo %>% 
    html_elements(css=".listing-thumb") %>% 
    html_attr("href")
  
  date = pageLinkNo %>% 
    html_elements(css="h6") %>% 
    html_text()
  date = str_trim(date)
  ...
  for (i in 1:length(articleLinks)) {
    ...
    name = articlePage %>%
      html_elements(css=".details-info-heading .rs-readable") %>%
      html_text()
    articlename = str_trim(name)[1]
    print(name)
    
    text = articlePage %>% 
      html_elements(css=".rs-readable p") %>% 
      html_text2()
...
write.csv(articleData, "Vooruit.csv", row.names = FALSE)

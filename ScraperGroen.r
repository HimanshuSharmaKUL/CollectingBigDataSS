...
link = "https://www.groen.be/nieuws?page="
baselink = "https://www.groen.be"
page = read_html(link)
...
css_selectors = 'div.blogCard.small.klimaat.kinderen_en_jongeren.onderwijs.mobiliteit.hasImage, div.blogCard.large.hasImage, div.blogCard.small.gezondheid.hasImage, div.blogCard.small.klimaat.hasImage, div.blogCard.small.natuur.overal_natuur.hasImage, div.blogCard.small.natuur.hasImage, div.blogCard.small.hasImage'
...
for (j in seq(1,NoOfPage)){
  ...
  articleLinks = pageLinkNo %>% 
                  html_nodes(css = css_selectors) %>%
                  html_elements('a') %>% 
                  html_attr("href")
  
  date = pageLinkNo %>% 
          html_elements(css=".date-span") %>% 
          html_text()
  date = str_trim(date)
  ...
  for (i in 1:length(articleLinks)) {
    ...
    name = articlePage %>%
      html_elements(css=".headline") %>%
      html_text()
    articlename = str_trim(name)[1]
    print(name)
    
    text = articlePage %>% 
      html_elements(css=".col-md-6 p") %>% 
      html_text2()
...
write.csv(articleData, "Groen.csv", row.names = FALSE)
